...
\centering\small guidance, navigation, \& control \hskip 3mm$\circ$ \hskip 3mm public policy \hskip 3mm$\circ$ \hskip 3mm machine learning \hskip 3mm$\circ$ \hskip 3mm systems biology
\vskip-6pt
\hrulefill
\end{adjustwidth}  
\reversemarginpar 
...
%\begin{itemize*}
%   \item Wrote engineering requirements / system specifications for generative sound design for a large-scale interactive art project (\$30\textsc{k} budget, ~20 volunteers)
...
\begin{itemize*}
  ...
  \item \textsc{Gn\&c} designs and analyses for specifying hardware requirements, systems engineering
  ...
  %\item Systems engineering: \textsc{gn\&c} systems, optical navigation (computer vision), and interplanetary radios (LunaNet)
\end{itemize*}
...
\begin{itemize*}
  ...
  \item Performed observability analysis of gimbaled single-axis rate-integrating gyroscope for gyrocompassing (joint motion kinematics)
%  \item Mission hardware and software support, wrote measurement models and extended Kalman filter for
  ...
  \item Terrain-relative (optical) navigation design
...
% 
  \item Designed, implemented, and documented multiple dual-state extended Kalman filters, batch least squares estimators, and complementary filters for aircraft (rotating frame), spacecraft (inertial frame), and drilling systems
...
\begin{itemize*}
  ...
  \item Expert recommendation systems, cross-validation
...
%  \item Served as \textsc{ta} for Biochemistry, Introduction to Bioinformatics (graduate level), and Chemistry for non-science majors
  \item Rewrote curriculum in Python (previously in Perl), largely focused on string processing and search algorithms, hidden Markov models, etc.
\end{itemize*}
...
\begin{itemize*}
  \item Multi-lingual noun phrase identification for abstract generation and machine translation
  \item Part of speech tagging, hidden Markov models
\end{itemize*}
...
\begin{itemize*}
  ...
  \item Statistical analyses on public health information
\end{itemize*}
